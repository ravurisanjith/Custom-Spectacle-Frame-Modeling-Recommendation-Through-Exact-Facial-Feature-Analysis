...
GLASSES_LIST = ["aviator", "round", "rectangular", "cateye", "large"]
selected_glasses = None
# Face shape to recommended glasses
FACE_SHAPE_GLASSES = {
    "Round": "rectangle",
    "Oval": "aviator",
    "Square": "round",
...
def auto_recommend():
    ...
    return render_template("auto_recommend.html")
@app.route("/random_glasses", methods=["POST"])
def random_glasses():
    global selected_glasses
    selected_glasses = random.choice(GLASSES_LIST)
    return jsonify({"status": "ok", "glasses": selected_glasses})
...
# -----------------------------------
@app.route("/reset_glasses", methods=["POST"])
def reset_glasses():
    global selected_glasses
    selected_glasses = None
...
# auto recommend glasses
    recommended_glasses = FACE_SHAPE_GLASSES.get(face_shape, "aviator")

# set selected glasses automatically
    global selected_glasses
    if selected_glasses is None:    
        selected_glasses = recommended_glasses
    ...
    "measurements": {
        "jaw_width": float(features[0]),
        "face_height": float(features[1]),
        "eye_distance": float(features[2]),
        "cheek_width": float(features[3]),
        "temple_width": float(features[4]),
        "face_ratio": float(features[5])
    },

    "frame_width": float(features[4] * 0.85)
})
